...
export default class AuthService {
  ...
  async decodeToken(req: Request) {
    ...
    if (typeof auth !== 'undefined') {
      ...
      const info = jwt.decode(token) as { data:{dataInfoUser:{email:string} }, exp:number };

      console.log(token, "next");
      // toLocaleString()
      if (info != null) {
        const dateTemp = info.exp;
        const dateCurrent = new Date(parseInt(`${dateTemp}000`));

        if (dateCurrent >= (new Date())) {
          return info.data.dataInfoUser;
        }
